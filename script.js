...
function waText(form){
  const f=new FormData(form),en=isEN();
  const seats=f.get('child_seats')||'0';
  const lines=en?[
    ...
    `Child seats: ${seats}`,`Luggage: ${f.get('luggage')||''}`,`Flight number: ${f.get('flight')||''}`
  ]:[
    ...
    `Kindersitze: ${seats}`,`Gepäck: ${f.get('luggage')||''}`,`Flugnummer: ${f.get('flight')||''}`
  ];
...
$$('[data-inquiry-form]').forEach(form=>{const d=form.querySelector('[data-return-trip]');d?.addEventListener('toggle',()=>{if(!d.open)return;const p=form.querySelector('[name=pickup]')?.value||'',z=form.querySelector('[name=destination]')?.value||'',rp=form.querySelector('[name=return_pickup]'),rz=form.querySelector('[name=return_destination]');if(rp&&!rp.value)rp.value=z;if(rz&&!rz.value)rz.value=p;});form.addEventListener('submit',e=>{e.preventDefault();window.open(`https://wa.me/${SD.whatsapp}?text=${encodeURIComponent(waText(form))}`,'_blank','noopener');});});
if('IntersectionObserver' in window){const obs=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting)e.target.classList.add('is-visible')}),{threshold:.08});$$('.reveal,.vehicle-card').forEach(x=>obs.observe(x));}else{$$('.reveal,.vehicle-card').forEach(x=>x.classList.add('is-visible'));}
